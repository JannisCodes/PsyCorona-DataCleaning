...
dt0Raw <- plyr::rbind.fill(dt0RawRUG, dt0RawNYUAD, dt0RawRepNYUAD, dt0RawRepRUG, dt0RawIran, dt0RawHNATD, dt0RawFly)
...
# merge (can also be dt0Raw but for now seperate)
  dt0All <- list(dt5newVars, dt0w1, dt0w2, dt0w3, dt0w4, dt0w5) %>% #merge multiple dfs at once
...
country_dict <- list('Afghanistan' = c('Afghanistan'),
                     ...
                     'Egypt' = c('مصر', '(?i)Egypt', 'المنوفيه', 'شبينالكوم', 'المنوفية', 'القاهره', 'منوف', 'تلا'),
                     ...
                     'Turkey' = c('تركيا - اسطنبول', 'turkey', 'تركيا', 'Tür', 't.rk..', 'Турция',  'Türkıye', '^Tr$', '^TR$', 'tr$', 'turki', 'turquie', 'TÜRKİYE', 'tur.iye', 'Türkiye', 't.rk.y.', 't.rk.{0,2}', "t[a-z]rkiye", 'турция'), #"'\u00fcrkiye'"
...
# save for Solomiia
  namCtrySol <- paste0("data/cleaned data/Solomiia Country List ", format(Sys.time(), format = "%F %H-%M %Z"),".csv")
  write.csv(namCtrySol, file = country_matches$unmatched)
...
```{r happy,echo=T, warning=F, message=F}
...
```{r mergWeight,echo=T, warning=F, message=F}
...
# Make contact list of all people before cutoff date
  ...
  filter(FilterPreview == 0,
         ymd_hms(dt5newVars$EndDate) < "2020-05-12 12:00:00 UTC",
...
# download contact lists from qualtrics
  CL1 <- read.csv("data/raw data/contact list/c19Recontact4_1.csv", header = T)
  CL2 <- read.csv("data/raw data/contact list/c19Recontact4_3.csv", header = T)
  CL3 <- read.csv("data/raw data/contact list/c19Recontact4_4.csv", header = T)
  CL <- plyr::rbind.fill(CL1, CL2, CL3)
