...
# Import RuG Snowball
dt0RawRUG <- read_spss("data/raw data/Snowball+Corona+Long-Term+Coverage+-+Baseline_April+21,+2020_10.45.sav")
dt0RawRUG$source <- "RUG"
# RUG Representative sample
dt0RawRepRUG <- read_spss("data/raw data/Snowball_Corona_Long-Term_Coverage_Baseline_RepresentativeRUG_April+21,+2020_10.26.sav")
dt0RawRepRUG$source <- "Rep RUG"
dt0RawRepRUG$country <- dt0RawRepRUG$country_new #fixed here but later in MTurk
# NYU Snowball
dt0RawNYUAD <- read_spss("data/raw data/Snowball Corona Long-Term Coverage - Baseline - Copy_April 24, 2020_03.08.sav")
dt0RawNYUAD$source <- "NYU-AD"
# NYU representative
dt0RawRepNYUAD <- read_spss("data/raw data/Snowball Corona - Baseline - Qualtrics REPRESENTATIVE_April 23, 2020_07.29.sav")
...
# Import Wave 1
  dt0w1 <- read_spss("data/raw data/Snowball+Corona+Long-Term+Coverage+-+Wave+2+April+3th_April+14,+2020_17.22.sav")
  dt0w2 <- read_spss("data/raw data/Snowball+Corona+Long-Term+Coverage+-+Wave+3+April+11th_April+14,+2020_15.04.sav")
...
# merge (can also be dt0Raw but for now seperate)
  dt0All <- list(dt0Raw, dt0w1, dt0w2) %>% #merge multiple dfs at once
...
cat("How many people did we loose in wave 2:")
  nrow(dt0w1) - sum(dt0All$ICRec_1_TEXT == dt0All$w1_RecipientEmail, na.rm = T)
...
# included
  inclRepNYU <- dt5newVars %>%
    filter(FilterPreview == 0,
           ...
           source == "Rep NYU-AD")
# excluded
  exclRepNYU <- dt5newVars %>%
    filter((FilterPreview == 1 |
           FilterProgress == 1 |
           FilterTime == 1 |
           FilterStraightliner == 1),
           source == "Rep NYU-AD")
# excluded (only straightliners)
  tmp <- dt5newVars %>%
    filter(FilterPreview == 0,
           Finished == 1,
           FilterTime == 0,
           source == "Rep NYU-AD")
  tmp2 <- tmp %>%
    filter(FilterStraightliner == 1)
    
# excluded
  allRepNYU <- dt5newVars %>%
    filter(source == "Rep NYU-AD")
```
...
country_dict <- list('Afghanistan' = c('Afghanistan'),
                     ...
                     'Brunei' = c('bruneidarussalam', 'br^unei'),
...
ctry.scales <- shiny_prep %>%
  ...
  dplyr::summarize(
    ...
    comRule = mean(c19IsStrict, na.rm = T)+rnorm(1, mean=0, sd=.2),
    comRule.sd = sd(c19IsStrict, na.rm = T),
    comRule.se = comRule.sd/sqrt(n),
    
    comPunish = mean(c19IsPunish, na.rm = T)+rnorm(1, mean=0, sd=.2),
    comPunish.sd = sd(c19IsPunish, na.rm = T),
    comPunish.se = comPunish.sd/sqrt(n),
    
    comOrg = mean(c19IsOrg, na.rm = T)+rnorm(1, mean=0, sd=.2),
