...
### Country
We currently have `r length(table(dt4newVars$country))` different free text country responses. Here we aim to consolidate them into one variable.
...
geolocations <- maxmind(dt4newVars$IPAddress, file, c("continent_name", "country_code", "country_name"))
dt4newVars$coded_country_ip <- geolocations$country_name
...
country_matches <- cat_words(tolower(dt4newVars$country), country_dict)
# Check duplicates, and which regular expressions triggered them
country_matches$dup
# Check unmatched strings, fix common ones
head(country_matches$unmatched)
dt4newVars$coded_country_cj <- country_matches$words
...
We currently have `r length(unique(dt4newVars$country))` different free text country responses. The most recent codes leaves `r sum(country_counts$n[country_counts$country != ""])` responses are still not consolidated.
</div>
...
pairs.panels.new(dt4newVars %>% dplyr::select(affAnx, affNerv))

cat("<br>")

dt4newVars$affHighNeg.m <- scoreItems(keys=c(1,1), items = dt4newVars %>% dplyr::select(affAnx, affNerv), min = 1, max = 5)$scores
...
dt4newVars$affHighNeg.fa <- fa(dt4newVars %>% dplyr::select(affAnx, affNerv))$scores
```
...
as.data.frame(psych::describe(dt4newVars %>% dplyr::select(affBor, affExh, affDepr))) %>%
...
pairs.panels.new(dt4newVars %>% dplyr::select(affBor, affExh, affDepr))

cat("<br>")

dt4newVars$affLowNeg.m <- scoreItems(keys=c(1,1,1), items = dt4newVars %>% dplyr::select(affBor, affExh, affDepr), min = 1, max = 5)$scores
...
dt4newVars$affLowNeg.fa <- fa(dt4newVars %>% dplyr::select(affBor, affExh, affDepr))$scores
```
...
as.data.frame(psych::describe(dt4newVars %>% dplyr::select(affCalm, affContent, affRel))) %>%
  mutate(vars = rownames(.)) %>%
...
pairs.panels.new(dt4newVars %>% dplyr::select(affCalm, affContent, affRel))

cat("<br>")

dt4newVars$affLowPos.m <- scoreItems(keys=c(1,1,1), items = dt4newVars %>% dplyr::select(affCalm, affContent, affRel), min = 1, max = 5)$scores
...
dt4newVars$affLowPos.fa <- fa(dt4newVars %>% dplyr::select(affCalm, affContent, affRel))$scores
```
...
as.data.frame(psych::describe(dt4newVars %>% dplyr::select(affEnerg, affExc, affInsp))) %>%
  mutate(vars = rownames(.)) %>%
...
pairs.panels.new(dt4newVars %>% dplyr::select(affEnerg, affExc, affInsp))

cat("<br>")

dt4newVars$affHighPos.m <- scoreItems(keys=c(1,1,1), items = dt4newVars %>% dplyr::select(affEnerg, affExc, affInsp), min = 1, max = 5)$scores
...
dt4newVars$affHighPos.fa <- fa(dt4newVars %>% dplyr::select(affEnerg, affExc, affInsp))$scores
```
...
as.data.frame(psych::describe(dt4newVars %>% dplyr::select(starts_with("lone")))) %>%
  mutate(vars = rownames(.)) %>%
...
pairs.panels.new(dt4newVars %>% dplyr::select(starts_with("lone")))

cat("<br>")

dt4newVars$lone.m <- scoreItems(keys=c(1,1,1), items = dt4newVars %>% dplyr::select(starts_with("lone")), min = 1, max = 5)$scores
...
dt4newVars$lone.fa <- fa(dt4newVars %>% dplyr::select(starts_with("lone")))$scores
```  
...
pairs.panels.new(dt4newVars %>% dplyr::select(starts_with("bor0"), -bor03))
```
<div class="alert alert-info">
...
pairs.panels.new(dt4newVars %>% dplyr::select(starts_with("bor0"), -bor03, -bor03_R))

cat("<br>")

dt4newVars$bor.m <- scoreItems(keys=c(1,1), items = dt4newVars %>% dplyr::select(starts_with("bor0"), -bor03, -bor03_R), min = -3, max = 3)$scores
...
dt4newVars$bor.fa <- fa(dt4newVars %>% dplyr::select(starts_with("bor0"), -bor03, -bor03_R))$scores
```
...
as.data.frame(psych::describe(dt4newVars %>% dplyr::select(ends_with("inPerson")))) %>%
  mutate(vars = rownames(.)) %>%
...
pairs.panels.new(dt4newVars %>% dplyr::select(ends_with("inPerson")))

cat("<br>")

dt4newVars$isoPers.m <- scoreItems(keys=c(1,1,1), items = dt4newVars %>% dplyr::select(ends_with("inPerson")), min = 0, max = 7)$scores
...
dt4newVars$isoPers.fa <- fa(dt4newVars %>% dplyr::select(ends_with("inPerson")))$scores
...
pairs.panels.new(dt4newVars %>% dplyr::select(ends_with("online")))

cat("<br>")

dt4newVars$isoOnl.m <- scoreItems(keys=c(1,1,1), items = dt4newVars %>% dplyr::select(ends_with("online")), min = 0, max = 7)$scores
...
dt4newVars$isoOnl.fa <- fa(dt4newVars %>% dplyr::select(ends_with("online")))$scores
...
as.data.frame(psych::describe(dt4newVars %>% dplyr::select(starts_with("extC19"), -extC19Msg))) %>%
  mutate(vars = rownames(.)) %>%
...
pairs.panels.new(dt4newVars %>% dplyr::select(starts_with("extC19"), -extC19Msg))

cat("<br>")

dt4newVars$ext.m <- scoreItems(keys=c(1,1,1), items = dt4newVars %>% dplyr::select(starts_with("extC19"), -extC19Msg), 
                             min = 1, max = 6)$scores
...
dt4newVars$ext.fa <- fa(dt4newVars %>% dplyr::select(starts_with("extC19"), -extC19Msg))$scores
```
...
as.data.frame(psych::describe(dt4newVars %>% dplyr::select(starts_with("c19per")))) %>%
  mutate(vars = rownames(.)) %>%
...
pairs.panels.new(dt4newVars %>% dplyr::select(starts_with("c19per")))

cat("<br>")

dt4newVars$beh.m <- scoreItems(keys=c(1,1,1), items = dt4newVars %>% dplyr::select(starts_with("c19per")), 
                             min = -3, max = 3)$scores
...
dt4newVars$beh.fa <- fa(dt4newVars %>% dplyr::select(starts_with("c19per")))$scores
```
...
as.data.frame(psych::describe(dt4newVars %>% dplyr::select(starts_with("para")))) %>%
  mutate(vars = rownames(.)) %>%
...
pairs.panels.new(dt4newVars %>% dplyr::select(starts_with("para")))

cat("<br>")

dt4newVars$para.m <- scoreItems(keys=c(1,1,1), items = dt4newVars %>% dplyr::select(starts_with("para")), 
                             min = 0, max = 10)$scores
...
dt4newVars$para.fa <- fa(dt4newVars %>% dplyr::select(starts_with("para")))$scores
```
...
as.data.frame(psych::describe(dt4newVars %>% dplyr::select(starts_with("consp")))) %>%
  mutate(vars = rownames(.)) %>%
...
pairs.panels.new(dt4newVars %>% dplyr::select(starts_with("consp")))

cat("<br>")

dt4newVars$consp.m <- scoreItems(keys=c(1,1,1), items = dt4newVars %>% dplyr::select(starts_with("consp")), 
                             min = 0, max = 10)$scores
...
dt4newVars$para.fa <- fa(dt4newVars %>% dplyr::select(starts_with("consp")))$scores
```
...
as.data.frame(psych::describe(dt4newVars %>% dplyr::select(starts_with("jbInsec"), -jbInsec02, -jbInsec04) %>% na_if(., -99))) %>%
  mutate(vars = rownames(.)) %>%
...
pairs.panels.new(dt4newVars %>% dplyr::select(starts_with("jbInsec"), -jbInsec02, -jbInsec04) %>% na_if(., -99))

cat("<br>")

dt4newVars$jobinsec.m <- scoreItems(keys=c(1,1,1), 
                                  items = dt4newVars %>% dplyr::select(starts_with("jbInsec"), -jbInsec02, -jbInsec04) %>% na_if(., -99),
                                  min = -2, max = 2)$scores
...
dt4newVars$jobinsec.fa <- fa(dt4newVars %>% dplyr::select(starts_with("jbInsec"), -jbInsec02, -jbInsec04))$scores
```
...
as.data.frame(psych::describe(dt4newVars %>% dplyr::select(starts_with("PFS0")))) %>%
  mutate(vars = rownames(.)) %>%
...
pairs.panels.new(dt4newVars %>% dplyr::select(starts_with("PFS0")))

cat("<br>")

dt4newVars$pfs.m <- scoreItems(keys=c(1,1,1), 
                                  items = dt4newVars %>% dplyr::select(starts_with("PFS0")),
                                  min = -2, max = 2)$scores
...
dt4newVars$pfs.fa <- fa(dt4newVars %>% dplyr::select(starts_with("PFS0")))$scores
```
