...
dt0RawRUG <- read_spss("data/raw data/Snowball+Corona+Long-Term+Coverage+-+Baseline_April+7,+2020_14.44.sav")
dt0RawRUG$source <- "RUG"

dt0RawNYUAD <- read_spss("data/raw data/Snowball Corona Long-Term Coverage - Baseline - Copy_April 8, 2020_05.57.sav")
...
as.data.frame(psych::describe(dt5newVars %>% dplyr::select(starts_with("fail")) %>% na_if(., -99))) %>%
  mutate(vars = rownames(.)) %>%
...
pairs.panels.new(dt5newVars %>% dplyr::select(starts_with("fail")) %>% na_if(., -99))

cat("<br>")

dt5newVars$disemp.m <- scoreItems(keys=c(1,1,1), 
                                  items = dt5newVars %>% dplyr::select(starts_with("fail")) %>% na_if(., -99),
                                  min = -2, max = 2)$scores
...
dt5newVars$disemp.fa <- fa(dt5newVars %>% dplyr::select(starts_with("fail")))$scores
```

### Societal Discontent
```{r socDis, echo=T, results='asis', warning=F, message=F}
ia.socdisc<- dt5newVars %>%
    dplyr::select(starts_with("disc"), -discPers , -disc03 ) %>%
    na_if(., -99) %>%
    Scale::Scale() %>%
    Scale::ItemAnalysis()
ia.socdisc$rely   
...
as.data.frame(psych::describe(dt5newVars %>% dplyr::select(starts_with("disc"), -discPers , -disc03) %>% na_if(., -99))) %>%
  mutate(vars = rownames(.)) %>%
...
pairs.panels.new(dt5newVars %>% dplyr::select(starts_with("disc"), -discPers , -disc03) %>% na_if(., -99))

cat("<br>")

dt5newVars$socdisc.m <- scoreItems(keys=c(1,1,1), 
                                  items = dt5newVars %>% dplyr::select(starts_with("disc"), -discPers , -disc03) %>% na_if(., -99),
                                  min = -2, max = 2)$scores
...
dt5newVars$socdisc.fa <- fa(dt5newVars %>% dplyr::select(starts_with("disc"), -discPers , -disc03))$scores
```
...
ia.probSolv<- dt5newVars %>%
    dplyr::select(starts_with("probSolving")) %>%
    Scale::Scale() %>%
    Scale::ItemAnalysis()
ia.probSolv$rely   
cat("<br><br>A gls factor analysis was conducted. Items were regressed to a single factor. Their loadings are the following:")
...
as.data.frame(psych::describe(dt5newVars %>% dplyr::select(starts_with("probSolving")))) %>%
  mutate(vars = rownames(.)) %>%
...
pairs.panels.new(dt5newVars %>% dplyr::select(starts_with("probSolving")))

cat("<br>")

dt5newVars$probSolv.m <- scoreItems(keys=c(1,1,1), 
                                  items = dt5newVars %>% dplyr::select(starts_with("probSolving")),
                                  min = -2, max = 2)$scores
...
dt5newVars$probSolv.fa <- fa(dt5newVars %>% dplyr::select(starts_with("probSolving")))$scores
```

#### Distraction
```{r distract, echo=T, results='asis', warning=F, message=F}
ia.distract<- dt5newVars %>%
    dplyr::select(starts_with("posrefocus")) %>%
    Scale::Scale() %>%
    Scale::ItemAnalysis()
ia.distract$rely   
cat("<br><br>A gls factor analysis was conducted. Items were regressed to a single factor. Their loadings are the following:")
...
as.data.frame(psych::describe(dt5newVars %>% dplyr::select(starts_with("posrefocus")))) %>%
  mutate(vars = rownames(.)) %>%
...
pairs.panels.new(dt5newVars %>% dplyr::select(starts_with("posrefocus")))

cat("<br>")

dt5newVars$distract.m <- scoreItems(keys=c(1,1,1), 
                                  items = dt5newVars %>% dplyr::select(starts_with("posrefocus")),
                                  min = -2, max = 2)$scores
...
dt5newVars$distract.fa <- fa(dt5newVars %>% dplyr::select(starts_with("posrefocus")))$scores
```
