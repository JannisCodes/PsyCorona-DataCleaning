...
dt3Straightliner <- merge(x=dt3Straightliner, y=qualtricsLanguage, by="Q_Language", all.x=TRUE)
rm(qualtricsLanguage)
```

### Country
We currently have `r length(table(dt3Straightliner$country))` different free text country responses. Here we aim to consolidate them into one variable.
...
dt3Straightliner <- dt3Straightliner %>%
  mutate(coded_country = case_when(grepl('^usa$', country, ignore.case = T) | grepl('unites state', country, ignore.case = T) | 
                                   ...
                                   grepl('hong kong', country, ignore.case = T) ~ 'Hong Kong',
                                   ...
                                   grepl('Srbija', country, ignore.case = T) | grepl('serbia', country, ignore.case = T) |
                                     grepl('Србија', country, ignore.case = T) ~ 'Serbia',
                                   ...
                                   grepl('costa', country, ignore.case = T) ~ 'costa', 
...
We currently have `r length(unique(dt3Straightliner$country))` different free text country responses. The most recent codes leaves `r sum(country_counts$n[country_counts$country != ""])` responses are still not consolidated.
</div>
