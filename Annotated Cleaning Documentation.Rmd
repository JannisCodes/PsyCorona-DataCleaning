...
dt0RawNYUAD <- read_spss("data/raw data/Snowball Corona Long-Term Coverage - Baseline - Copy_April 14, 2020_06.56.sav")
...
# merge
  tmp <- list(dt0Raw, dt0w2, dt0w3) %>% #merge multiple dfs at once
...
cat("How many people did we loose in wave 2:")
  nrow(dt0w2) - sum(tmp$ICRec_1_TEXT == tmp$w2_RecipientEmail, na.rm = T)
  cat("We lost them because an independent link was sent around")
  
cat("How many people did we loose in wave 3:")
  nrow(dt0w3) - sum(tmp$ICRec_1_TEXT == tmp$w3_RecipientEmail, na.rm = T)
  cat("We lost them because an independent link was sent around")
...
country_dict <- list('Afghanistan' = c('Afghanistan'),
                       ...
                       'Algeria' = c('Algrie', 'الجزاء و', 'ALGERIE',  'جزائر',   'Alger',  'الجزاء و ', 'الجزاير',  '🇩🇿', 'ALGÉRIE',  'البليدة',  'algeri', 'الجزاىر',  'الجوائر',    'Algeria', 'الحزائر',  'الجزائر', 'algérie', 'الجزاءر', 'Algérie', 'Algerie'),
                       'Algeria' = c('الجزاير (حمادي بومرداس)', 'الحزاءر'),
                       'Andorra' = c('andorra', 'Andorra'),
                       'Argentina' = c('Αργεντινή', 'argentina', 'Argentine', 'ARGENTINA', 'Argentina', 'arge'),
                       'Australia' = c('australia', 'austrlaia',  'austrija', 'Australia'),
                       'Austria' = c('Австрія', 'Asturies', 'austria', 'Austrija', 'Austria', 'sterreich', 'Australia'),
                       'Azerbaijan' = c('azerba', 'Azarbaycan',  'Azerbaycan', 'Azerbaijan'),
                       'Bahrain' = c('bahrain', 'Bahrain'),
                       'Bangladesh' = c('bangladesh', 'Bangladesh'),
                       'Belgium' = c('Βελγιο', 'βελγιο', 'Belgien', 'Бельгия', 'Bélgica', 'Βελγιο', 'βελγιο', 'Belgica', 'Belgio', 'Belgie', 'België', 'belg', 'Belgique', 'bélgica', 'бельгия', 	'Belgium'),
                       ...
                       'Bulgaria' = c('bulgaria', 'Bulgaria', 'българия'),
                       'Cambodia' = c('Cambodia'),
                       'Cameroon' = c('camer', 'Cameroun'),
                       'Canada' = c('canad', 'کانادا', 'Windsor', 'Québec', 'Ontario', 'Vancouver', 'vancouver', 'КАНАДА', 'CANADA', 'Canadá', 'Канада',  'канада', 'Canada'),
                       'Chile' = c('^chile$', 'CHile',  'Chile', 'CHILE'),
                       'China' = c('^china','Macao', 'Macau',  'Hui', 'Chine', '山东泰安东平县', '中国', '中华人民共和国', 'China', '中國'),
                       'Colombia' = c('colombia', 'Colombia', 'COLOMBIA'),
                       'Costa Rica' = c('costa', 'Costa rica', 	'Costa Rica'), 
                       'Croatia' = c('croa', 'Kroatien', 'Horvátország',  'Croácia', 'Croazia',  'HRVATSKA', 'Hrvatskoj', 'Hrv', 	'Hrvarska', 'Hrvatska', "hrvatsk", 'Croatia'),
                       ...
                       'Czech Republic' = c('czech', 'Τσεχία', 'Republica Checa', 'češka republika', 'Tschechien',  'República Checa',  'Czechia',  'Czech Republic', 'checa'),
                       'Denmark' = c('mark'), 
                       ...
                       'France' = c('france', 'Vendée', 'Valmontone (RM)', 'vouziers', 'Région PACA', 'Paris', 'París', 'Oise', 'nouvelle-calédonie', 'LYON', 'Guyane Française', 'Guadeloupe', 'Grand est', 'Fransa', 'Frznce', 'Réunion', 'Franche', 'Frankreich', 'França',  'Frankrijk', '^fr$', 'La Réunion', 'La réunion', 'Francia', 'France', 'FRANCE', 'francia',  'frankrijk'),
                       'Gabon' = c('Gabon'),
                       'Germany' = c('Duitsland', 'Németország', 'Nemačka', 'Nemacka', 'Germanis', 'ΓΕΡΜΑΝΙΑ', 'آلمان', 'Германия', 'ألمانيا', 'Berlin', 'Γερμανια', 'Allemagne', 'germany', 'Γερμανία', 'Γερμανία', 'Almanya', 'Alemania', 'Germania',  'Germany', 'Deutschland', 'deutschland',  'alemania',  'germania',  '^almanya$', 'allemagne',  'berlin',  'duitsland', 'γ.ρ.....'),
                       'Greece' = c('ΕΛΛΑΣΑ', 'Ελλα', 'Ιωάννινα', 'greece', 'Θεσσαλονίκη', 'Hellas', 'GRecia', 'Griechenland', 'Griekenland',  'Grèce',  'Ellada', '希腊', 'GRECE', 'ellada', 'Grecia', 'GREECE', 'Ελλα', 'ΕΛΛΑΣ', 'ΕΛΛΑΣΑ', 'ΕΛΛΆΔΑ', 'Greece', 'ΕΛΛΑΔΑ', 'grecia', 	'.λλ...', '.λλ..', 'ελλάδα'),
                       'Guatemala' = c('guat', 'Guatemala'),
                       'Hong Kong S.A.R.' = c('Hongkong', 'Hk', 'HK', 'HONG KONG', 'HongKong', 'Hong kong',  'hong.{0,}kong', '香港', 'Hong Kong'),
                       'Hungary' = c('Hungary', 'Ungheria', 'Ungarn', 'Mađarska', 'hungary', 'Magyar', 'Magyarorszag', 'Magyaroszág', 'Magyarország', '^magyarorsz', 'magyar', 'ungarn'),
                       'Iceland' = c('iceland', 'Iceland'),
                       'India' = c('india', 'هند', 'Imdia', 'INDIA', 'Infis',  'India'),
                       'Indonesia' = c('Jakarta', 'Indomesia', 'Insonesia', 'Indònesia', 'Yogyakarta, indonesia', 'Indonesià', 'Indonedia', 'Indomesia	', 'Indoesia', 'Indobesia', 'Imdonesia', 'Indoasia', 'indo', 'INdonesia', 'INDONESIA', '^indonesia$', 'Indonesia', 'indonesian',  'kota, tarakan',  'imdonesia',  'indònesia',  'jakarta'),
                       'Iran' = c('iran', 'ایران', 'Iran'),
                       'Iraq' = c('عراق',  'iraq', 'الغراق', 'Iraq', 'العراق'),
                       'Ireland' = c('Ιρλανδία', 'Ιρλανδια', '^ireland$', 'Irska', 'Ireland', 'Irlanda', 'irlanda', 'ιρλ.....'),
                       'Israel' = c('عارة',   'سخنين',   'رمله',   'القدس','Isreal', 'الطيبة',  'Израиль', 'الطيبه',  'Jerusalem', 'كفر قاسم',  'الطيرة', 'ISRAEL', 'جت المثلث', 'Israel', 'جلجولية',  'israel', 'اللد',  'اسرائيل',  'كفر, قاسم',  'isreal',  'רמלה', 'الرمله'), 
                       'Italy' = c('Італія', 'Ιταλία', 'Terralba', 'Tezze di Piave (Tv) ITA', 'Vicenza', 'Vergato', 'Vittorio Veneto',  'SETTIMO VITTONE', 'SS. Cosma e Damiano', 'Segni (RM)', 'Scalea', 'Scauri', 'ROMA', 'Predazzo (Tn)', 'Pellizzano', 'Padova', 'Olevano romano',  'Orosei', 'Italua', 'Itaia', 'Imola', 'Ladispoli', 'Guidonia', 'Guidonia Montecelio', 'Ferentino', 'Fiano romano', 'Chieti', 'Cala gonone',  'Brescia',  'Bari',  'Италия',   'İtalya', 'Mareno di Piave', 'Frosinone', 'Italia', 'Milano', 'Dorgali', 'Castelforte', 'Bisceglie',  'Italie', 'Italië', 'Itali', 'Italien', 'Sardegna',  'ITALIA', 'ital', 'Roma', 'Italy', 'sardegna',  'bisceglie',  'ladispoli',  'castelforte',  'milano',  '^roma$', 'dorgali',  'bari',  'bologna',  'brescia',  'cala, gonone',  'chieti',  'ferentino',  'frosinone',  'gragnano, lucca, ',  'guidonia',  'itaia',  'italya',  'mareno, di, piave',  'modena',  'pellizzano',  'predazzo',  'refrontolo',  'cosma, e, damiano',  'scalea',  'scauri',  'segni',  'settimo, vittone',  'susegana',  'terralba',  'trento',  'treviso',  'tezze, di, piave',  'valmontone',  'vergato',  'veneto',  'gragnano, lucca'),
                       'Jamaica' = c('jamaica', 'Jamaica'),
                       'Japan' = c('japan', 'Japão', 'Japonya', 'にほん', 'japon', 'Giappone',  'giappone', 'Japan', 'japonya', '日本'),
                       'Jordan' = c(  'الاردن', 'Jordan',   'Jordanie',  'jordan', 'الأردن'),
                       'Kazakhstan' = c('казахстан', 'Qazaqstan', 'Kazahstan', 'Kazakhstan', 'kazakhstan', 'Казахстан'),
                       'Kenya' = c('kenia', 'Kenia'),
                       'Kuwait' = c('الكويت'),
                       'Laos' = c('laos', 'Laos'),
                       'Latvia' = c('latvia', 'Latvia'),
                       'Lebanon' = c('lebanon', 'LIBAN', 'Ливан',  'لبنان',  'Lebanon', 'liban'),
                       'Libya' = c('ليبيا'),
                       'Luxembourg' = c('lux','Luksemburg',  'Luxembourg', 'Luxemburg'),
                       'Malaysia' = c('malay', 'Malaysia', 'ماليزيا'),
                       'Mali' = c('Mali'),
                       'Malta' = c('malta', 'Malta'),
                       ...
                       'Morocco' = c('المغرب',  'rabat', 'النغرب',  'MAROC', 'مغرب',  'Morocco', 'Morroco', 'Maroko', 'Marruecos', 'maroc', 'Maroc'),
                       'Myanmar' = c('myanmar'),
                       'Namibia' = c('Namibia'),
                       'Nepal' = c('nepal', 'Nepal'),
                       'Netherlands' = c('Netherland','Nizozemska',  'Holandija',  'Gelderland', '荷兰', 'Нидерланды',  'Голандія', 'Paises Bajos', 'Curacao',  'Hollanda', 'Países Bajos', 'Paesi Bassi', 'netherland', 'Nl', 'Belanda', 'Olanda', 'New Zealand', 'NL', 'Niederlande', 'The Netherlands', 'Netherlands', 'Nederland',  'nederland',  'niederlande',  'belanda',  '^nl$',  'olanda',  'paesi, bassi',  'paesi bassi',  'bajos',  'gelderland',  'hollanda', 'paes.+bass.'),
                       'New Zealand' = c('new.+zeal',  'newzealand',  'neuseeland'),
                       'Nigeria' = c('nigeria', 'Nigeria'),
                       'North Macedonia' = c('North Macedonia'),
                       'Norway' = c('norway', 'Norway', 'Norvège', 'Norvége', 'Norwegen'),
                       'Oman' = c('^oman$', 'Oman'),
                       'Pakistan' = c('pakistan', 'پاکستان', 'Peshawar',  'Abbottabad', 'Pak',	'Pakistan',  'abbottabad',  'peshawar'),
                       ...
                       'Panama' = c('panam'), 
                       'Peru' = c('PERÚ', 'Peru', 'Perú', 'peru', 'perú'),
                       'Philippines' = c('Phillipines', 'Ph', 'Pilipinas', 'Phil', 'Philippinea', 'phil',  'filipinas', 'Philippines', 'PHILIPPINES'),
                       'Poland' = c('poland', 'POLAND', 'POLONIA', 'Portogallo', 'Poland',  'polonia', 'Польша', 'польша'),
                       'Portugal' = c('portugal', '葡萄牙', 'Portugal'), 
                       'Qatar' = c('qatar', 'Qatar', 'Katar'),
                       'Romania' = c('romania', 'ROMANIA', 'Roumanie',  'Romania', 'România', 'rom.nia'),
                       'Russia' = c('россия', 'Россия', 'Rusia'),
                       'Republic of Belarus' = c('Belarus'),
                       'Republic of Kosovo' = c('Kosovo'),
                       'Republic of Chad' = c('Tchad'),
                       'Republic of Serbia' = c('srbija', 'Serbis', 'Servië', 'SEBIA', 'У Србији',  'у Србији',  'Србијс', 'Szerbia', 'Srpski', 'Srbiji', 'SRBIJA',  'serbia',  'србија', 'Serbia','srbiji', 'Србија', 's.erbia', 'Srbija'),
                       'Rwanda' = c('Rwanda'),
                       'Saudi Arabia' = c('اريا',  'المدينة المنورة', 'المملكة', 'جازان',  'جده',  'SaudiArabia', 'جدة',  'Riyadh',  'Sa',    'سعوديه',  'Riyad', 'Saudi', 'K.S.A',  'Arabia Saudita', 'Arabie saoudite', 'Saidi Arabia', '	Saudi arbia', 'Ksa', 'Saudi Arabia', 'SA', 'riyad',  'saud.+arab',  'arabia, saudita',  '^saudi$',  'kingdom, of, saudia, arabia',  'ksa',  'k\\.s\\.a',  'arabie, saoudite',  'الرياض',  'السعودية',  'السعوديه', 'emira+arab'),
                       'Singapore' = c('singapore', 'SINGAPORE', 'Singapur', 'Singapore'),
                       'Slovakia' = c('slova', 'Словаччина',   'Szlovákia',  'Slovakia', 'Slovak Republic'),
                       'Slovenia' = c('Slovenia', 'Slovenija'),
                       'South Africa' = c('جنوب أفريقيا', 'Garankuwa','Western Cape',  'Limpopo', 'Western cape', 'Eastern Cape', 'Cape Town', 'Gauteng', 'SOUTH AFRICA', 'South aftica', 'South Afica', 'South AFRICA', 'South Africa', 'africa',  '^sa$',  'sudáfrica',  'western, cape', 'southaf...'),
                       'South Korea' = c('korea', '한국', 'Korea, Republic of', 'Güney Kore', 'Korea Selatan, Busan',  'South Korea', 'güney, kore'),
                       'Spain' = c('Испания', 'Ισπανία', 'Pais vasco', 'Montalbán de Córdoba', 'Madrid', 'Málaga', 'Fuerteventura', 'Espña', 'Es', 'Esapaña', 'El Rocío', 'Eapaña', 'Cataluña', 'Andalucia', 'Catalunya', 'Espsña', 'Euskal Herria', 'Іспанія', 'Spanyolország', 'SPAIN', 'Estado Español', 'Españistan', 'ESpaña', 'Espanha', 'Espana', 'Spanje', 'Españs', 'Españ', 'Espagne', 'Spanien', 'Español', 'Espanya', 'spain', 'Spagna', 'Spain', 'esp', 'ESPAÑA', 'España', 'spagna',  'spanien',  'catal',  'euskal, herria',  'basque',  'eapaña',  'esapaña',  'madrid',  'montalbán, de, córdoba',  'pais, vasco',  'spanje'),
                       'Sweden' = c('weden','Svezia', 'SWEDEN',  'Isvec', 'Suède'),
                       'Switzerland' = c('Ελβετία', 'switzerl', 'Zwitserland', 'Switzerlannd', 'Svizzera','İsviçre',  'Швейцарія', 'Suiza', 'SUIZA', 'schweiz', 'Ελβετία', 'Schweiz', 'Switzerland', 'suiza', 'Suisse', 'svizzera',  'zwitserland',  'switzerland', 'suisse'),
                       'Taiwan' = c('taiwan', 'Tayvan', '台灣', 'Taiwan'),
                       ...
                       'Turkey' = c('تركيا - اسطنبول', 'turkey', 'تركيا', 'Tür', 'Türkçe', 'Türkye', 'Türkei', 'Türkiy', 'Турция',  'Türkıye', 'Tr', 'TR', 'tr', 'TURKEY', 'Turki', 'turquie', 'TÜRKİYE', 'Turkey', 'Turkiye', 'Türkiye', 't.rk.y.', 't.rk.{0,2}', "t[a-z]rkiye"), #"'\u00fcrkiye'"
                       'Ukraine' = c('ukraine', 'Ураїна', 'Киев', 'Укрїна', 'Украiна', 'Українська', 'Украине', 'УКРАЇНА', 'Украіна', 'у.ра..а', 'україна', 'украина', 'Україна', 'Украина'),
                       'United Arab Emirates' = c('U.A.E.',  'الإمارات العربية المتحدة' ,  'Uae' , 'Emirats Arabes Unis', 'Emiratos Arabes Unidos' ,'Abu Dhabi',  'Emiratele Arabe Unite' ,   'Dubai',   'Emirados Arabes Unidos', 'الخرج','United Arab emirates',  'United Arab Emirates',  'UAE', 'united, arab, emirates', 'الامارات', '^uae$', 'arabemirates', 'لعربيةالمتحدة'),
                       'United Kingdom' = c('Αγγλια', 'اسكوتلندا', 'Egland', 'royaume uni', 'Liverpool', 'Grand Cayman, Cayman Islands', 'GB', 'Anglia', 'Ecosse', 'Angleterre', 'Великобритания', 'Αγγλια', 'U.K','SCOTLAN', 'Schotland',  'Scozia',  'Ηνωμένο Βασίλειο', 'London', 'ENGLAND', 'Verenigd Koninkrijk', 'Αγγλία', 'Marea Britanie', 'Britain', 'Isole Vergini Britanniche',  'Great Britain',  'United Kingdon', 'Northern ireland', 'UNITED KINGDOM', 'united Kingdom', 'Regno Unito', 'United kingdom', 'United Kingdon', 'uni.+kin', 'Wales', 'Reino Unido', 'Uk', 'Scotland', 'United Kingdom', 'England', 'UK',  '^uk$',  'reino, unido',  'britain',  'regno, unito',  'u\\.k\\.',  'بريطانيا',  'the, uk',  'u, k',  'verenigd, koninkrijk',  'windsor',  'scotland',  'england',  'wales',  'ingiltere',  'northern, ireland',  'egland',  '^gb$',  'n, ireland',  'schotland',  'scozia', '英国', 'ngiltere', 'n.{4,9}ireland', 'reinounido', 'anglia'),
                       'United Republic of Tanzania' = c('tanzania', 'Tanzania'),
                       'United States of America' = c('فلوريدا',     'الولايات المتحدة الامريكية'  , 'الولايات المتحدة الأمريكية',   'Αμερική', 'U.s.a.', 'Tempe', 'TEXAS', 'untied states', 'TX', 'Untied STates', 'Wisconsin', 'virginia', 'illinois', 'Puerto Rico', 'Penssylvania', 'NY', 'new yark', 'Munford', 'michigan', 'midwest', 'Minnesota', 'EEUU', 'IDAHO', 'georgia', 'Amerika Serikat', 'AMERICA', 'ALASKA', '^us$', '미국', 'آمریکا', 'أمريكا', 'Pennsylvania', 'Pennssylvania', 'United stated', 'United stares', 'Estados unidos', 'NEWYORK', 'Arizona',  'New York', 'new york', 'New york', 'NEW YORK', 'США', 'new jersey',  'Unites stats', 'California', 'Uniteed States', 'United. States','United STates','United sTATES', 'uNITED STATES', 'United Stares', 'United Staes', 'Unite States', 'ESTADOS UNIDOS', 'Unites states', 'United Ststes', 'UNited States', 'united States', 'United statea', 'United Sates', 'UNITED STATES', 'Estados Unidos', '	Estados unidos', '	ESTADOS UNIDOS', 'Untied statest', 'Unites States', 'United State', 'U.S', 'United states', 'America', 'United States', 'Usa', 'USA', 'america', '^usa', 'u.s.a',  'uni.+stat', 'uni.+merica',  '^america$',  '^u.{0,1}s.{0,1}$',  'esta.+unid',  'colorado',  'xas',  'sates',  'amerika, serikat',  'california',  'corlifornia',  'états-unis', 'puerto, rico',  '^tx$',  '^tn$', '美国', 'new+york', 'сша', 'un..edsta.es', 'arizona'), 
                       ...
                       'United States of America' = c('امریکا'), 
                       'Saudi Arabia' = c('مدينة ضرماء',      'عنيزة',    'سكاكا الجوف',     'ضرماء',      '	سكاكا الجوف',  'سعودية',   'الدمام', 'الرباض', 'السعرديه', 'السعوديّه' ),
                       'Saudi Arabia' = c('رياض',  'خرخير',  'حفر الباطن',  'القويعيه',   'المدينه المنورة')
      )
