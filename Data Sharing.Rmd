...
# Import Qualtrics Survey Data
  ...
  dtAll <- read_spss("data/cleaned data/Psycorona Baseline cleaned - All 2020-05-11 19-34 CEST.sav")
...
# if there is a W1 (baseline) paste w1 and the variable
  ...
  if (grepl("W2|Wave2|Wave 2",df[2], ignore.case = T)) {
    stringOut[3] = paste("w2_", df[1], sep = "")
  } else {
    stringOut[3] = NA
  }
  if (grepl("W3|Wave3|Wave 3",df[2], ignore.case = T)) {
    stringOut[4] = paste("w3_", df[1], sep = "")
  } else {
    stringOut[4] = NA
  }
  if (grepl("W4|Wave4|Wave 4",df[2], ignore.case = T)) {
    stringOut[5] = paste("w4_", df[1], sep = "")
  } else {
    stringOut[5] = NA
  }
  if (grepl("W5|Wave5|Wave 5",df[2], ignore.case = T)) {
...
### RMD00 Leon needs lat/long
```{r bertDemo}
tmp <- dtAll %>%
  transmute(ResponseId = ResponseId,
            LocationLatitude = LocationLatitude,
            LocationLongitude = LocationLongitude)
write_sav(tmp, paste0("data/collab data/LeonLatLong ", format(Sys.time(), format = "%F %H-%M %Z"),".sav"))
write.csv(tmp, paste0("data/collab data/LeonLatLong ", format(Sys.time(), format = "%F %H-%M %Z"),".csv"))
rm(tmp)
```
...
# make representative if needed
  if(sum(grepl("source", names(dt))) == 1 ){ # if the sum of columns that have the string source == 2
    dt <- dt[grepl("Rep", dt$source),]  # reduce df to representative
  } else { # if not
    dt <- dt # keep dt
  }
...
### RMD20 (Agostini)
```{r RMD20, echo=T, warning=F, message=F}
# prepare dataframe
name <- 'RMD20'
# from here everything is automatic (just need to change the name above)
  author <- namedtReq %>%
