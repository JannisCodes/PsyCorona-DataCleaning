...
# Import Qualtrics Survey Data
  ...
  dtAll <- read_spss("data/cleaned data/Psycorona Baseline cleaned - All 2020-06-23 13-01 CEST.sav")
...
# if there is a W1 (baseline) paste w1 and the variable
  ...
  if (grepl("W11|Wave11|Wave 11|All",df[2], ignore.case = T)) {
    stringOut[11] = paste("w11_", df[1], sep = "")
  } else {
    stringOut[11] = NA
  }
return(stringOut)
...
# needs response ID (make through digest)
  dt$ResponseId <- c(1:nrow(dt))
  for (i in 1:nrow(dt)) {
  dt$ResponseId[i] <- digest::digest(dtAll$ResponseId[i], algo = "xxhash32", seed = 42)
  }
...
# needs response ID (make through digest)
  dt$ResponseId <- c(1:nrow(dt))
  for (i in 1:nrow(dt)) {
  dt$ResponseId[i] <- digest::digest(dtAll$ResponseId[i], algo = "xxhash32", seed = 43)
  }
...
# needs response ID (make through digest)
  dt$ResponseId <- c(1:nrow(dt))
  for (i in 1:nrow(dt)) {
  dt$ResponseId[i] <- digest::digest(dtAll$ResponseId[i], algo = "xxhash32", seed = 44)
  }
...
# needs response ID (make through digest)
  dt$ResponseId <- c(1:nrow(dt))
  for (i in 1:nrow(dt)) {
  dt$ResponseId[i] <- digest::digest(dtAll$ResponseId[i], algo = "xxhash32", seed = 45)
  }
...
# needs response ID (make through digest)
  dt$ResponseId <- c(1:nrow(dt))
  for (i in 1:nrow(dt)) {
  dt$ResponseId[i] <- digest::digest(dtAll$ResponseId[i], algo = "xxhash32", seed = 46)
  }
...
# needs response ID (make through digest)
  dt$ResponseId <- c(1:nrow(dt))
  for (i in 1:nrow(dt)) {
  dt$ResponseId[i] <- digest::digest(dtAll$ResponseId[i], algo = "xxhash32", seed = 48)
  }
...
# needs response ID (make through digest)
  dt$ResponseId <- c(1:nrow(dt))
  for (i in 1:nrow(dt)) {
  dt$ResponseId[i] <- digest::digest(dtAll$ResponseId[i], algo = "xxhash32", seed = 49)
  }
...
# needs response ID (make through digest)
  dt$ResponseId <- c(1:nrow(dt))
  for (i in 1:nrow(dt)) {
  dt$ResponseId[i] <- digest::digest(dtAll$ResponseId[i], algo = "xxhash32", seed = 50)
  }
