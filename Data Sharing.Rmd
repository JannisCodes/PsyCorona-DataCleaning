...
# Import Request sheet
  ...
  dtReqMeta <- dtReq[c(300:nrow(dtReq)),]
    ...
    if(nrow(tmp) == 0 ){ # if row length is 0
      ...
      # for loop to merge each dataset according to its merging mechanism (iso or more complicated)
        for (i in 1:nrow(tmp)) {
          ...
          # remove country columns if they were aded
            dt <- dt %>%
             dplyr::select(-matches('^country..$'),
                           -matches('^region..$'))
          }
        # Oxford data
          else if(tmp$mergTech[i] == "oxford") {
            print(paste("Merging ", tmp$name[i], ". Currently not working...", sep = ""))
        #   # load data
        #     dtMeta <- read.csv(tmp$path[i], stringsAsFactors = FALSE)
        #     # prepare merg data
        #       dtMeta$mergCtry <- dtMeta$CountryCode
        #       dtMeta$mergDate <- dtMeta$Date
        ...
        #     # merge
        #       dt <- merge(dt, dtMeta, by=c("mergCtry","mergDate"))
        #   # Wave 1
        #     dt$mergDate <- as.numeric(paste0(substr(dt$w1_EndDate , 1, 4), substr(dt$w1_EndDate , 6, 7),
        #                                      substr(dt$w1_EndDate , 9, 10)))
        #     # merge
        #       dt <- merge(dt, dtMeta, by=c("mergCtry","mergDate"))
        #   # Wave 2
        #     dt$w2_mergDate <- as.numeric(paste0(substr(dt$w2_EndDate , 1, 4), substr(dt$w2_EndDate , 6, 7),
        #                                         substr(dt$w2_EndDate , 9, 10)))
        #     # merge
        #       dt <- merge(dt, dtMeta, by=c("mergCtry","mergDate"))
        #   # Wave 3
        #     dt$w3_mergDate <- as.numeric(paste0(substr(dt$w3_EndDate , 1, 4), substr(dt$w3_EndDate , 6, 7),
        #                                         substr(dt$w3_EndDate , 9, 10)))
        #     # merge
        #       dt <- merge(dt, dtMeta, by=c("mergCtry","mergDate"))
        #   # Wave 4
        #     dt$w4_mergDate <- as.numeric(paste0(substr(dt$w4_EndDate , 1, 4), substr(dt$w4_EndDate , 6, 7),
        #                                         substr(dt$w4_EndDate , 9, 10)))
        #     # merge
        #       dt <- merge(dt, dtMeta, by=c("mergCtry","mergDate"))
        #   # Wave 5
        #     dt$w5_mergDate <- as.numeric(paste0(substr(dt$w5_EndDate , 1, 4), substr(dt$w5_EndDate , 6, 7),
        #                                         substr(dt$w5_EndDate , 9, 10)))
        #     # merge
        #       dt <- merge(dt, dtMeta, by=c("mergCtry","mergDate"))
        #   # Wave 6
        #     dt$w6_mergDate <- as.numeric(paste0(substr(dt$w6_EndDate , 1, 4), substr(dt$w6_EndDate , 6, 7),
        #                                         substr(dt$w6_EndDate , 9, 10)))
        #     # merge
        #       dt <- merge(dt, dtMeta, by=c("mergCtry","mergDate"))
        #   # Wave 7
        #     dt$w7_mergDate <- as.numeric(paste0(substr(dt$w7_EndDate , 1, 4), substr(dt$w7_EndDate , 6, 7),
        #                                         substr(dt$w7_EndDate , 9, 10)))
        #     # merge
        #       dt <- merge(dt, dtMeta, by=c("mergCtry","mergDate"))
        #   # Wave 8
        #     dt$w8_mergDate <- as.numeric(paste0(substr(dt$w8_EndDate , 1, 4), substr(dt$w8_EndDate , 6, 7),
        #                                         substr(dt$w8_EndDate , 9, 10)))
        #     # merge
        #       dt <- merge(dt, dtMeta, by=c("mergCtry","mergDate"))
        #   # Wave 9
        #     dt$w9_mergDate <- as.numeric(paste0(substr(dt$w9_EndDate , 1, 4), substr(dt$w9_EndDate , 6, 7),
        #                                         substr(dt$w9_EndDate , 9, 10)))
        #     # merge
        #       dt <- merge(dt, dtMeta, by=c("mergCtry","mergDate"))
        #   # Wave 10
        #     dt$w10_mergDate <- as.numeric(paste0(substr(dt$w10_EndDate , 1, 4), substr(dt$w10_EndDate , 6, 7),
        #                                          substr(dt$w10_EndDate , 9, 10)))
        #     # merge
        #       dt <- merge(dt, dtMeta, by=c("mergCtry","mergDate"))
        #   # Wave 11
        #     dt$w11_mergDate <- as.numeric(paste0(substr(dt$w11_EndDate , 1, 4), substr(dt$w11_EndDate , 6, 7),
        #                                          substr(dt$w11_EndDate , 9, 10)))
        #     # merge
        #       dt <- merge(dt, dtMeta, by=c("mergCtry","mergDate"))
        #   # Wave 12
        #     dt$w12_mergDate <- as.numeric(paste0(substr(dt$w12_EndDate , 1, 4), substr(dt$w12_EndDate , 6, 7),
...
```{r RMD56, echo=T, warning=F, message=F}
dataCreate("RMD56", 56)
```